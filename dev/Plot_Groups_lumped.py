# -*- coding: utf-8 -*-
"""
Created on Wed Oct 25 17:05:56 2023
...
plt_dir = os.path.join(current_dir, 'plt')
...
os.chdir(os.path.join(dat_dir, 'interim'))
n_exp = 40
fs = 2400
...
#os.chdir(r'J:\Publikationsliste\2025\Woody-Article\plt')                                
#n_exp = 10
date = "240219_"
#%% plot time and forces einzeln
tmin = 8000
tmax = 14400
fs = 2400
# Startzeile für die Suche nach Vorzeichenwechsel


# Indices to process
# Indices to process
indices = [1, 2, 3, 4, 5]
image_path = os.path.join(doc_dir, 'figures', 'Full_slab_nowood.png') #Setup A
#image_path = os.path.join(doc_dir, 'figures', 'Full_slab_wood20.png')
#indices = [6, 7, 8, 9, 10] #Setup B
#indices = [11, 12, 13, 14, 15] #Setup C
#indices = [16, 17, 18, 19, 20] #Setup D
#indices = [21, 22, 23, 24, 25] #Setup A*
#indices = [26, 27, 28, 29, 30] #Setup B*
#indices = [31, 32, 33, 34, 35] #Setup C*
#indices = [36, 37, 38, 39, 40] #Setup D*
#image_path = os.path.join(doc_dir, 'figures', 'Trough_wood20.png') #Setup A
...
# ab = AnnotationBbox(im, (0.66, 0.919), xycoords='axes fraction', bboxprops=dict(facecolor='white', edgecolor='lightgrey', boxstyle='round,pad=0.5', alpha=0.8))
# ax.add_artist(ab)
# fig.tight_layout()
# plt.show
...
# Plotting individual plots for each index
for x in range(len(indices)):
    ...
    match i:
        case 1:
            start_row = 8400
            image_path = os.path.join(doc_dir, 'figures', 'Full_slab_nowood.png')
        case 2:
            start_row = 8400
            image_path = os.path.join(doc_dir, 'figures', 'Full_slab_nowood.png')
        case 3:
        ...
        case 6:
            start_row = 9699
            image_path = os.path.join(doc_dir, 'figures', 'Full_slab_wood10.png')
        case 7:
            start_row = 9000
            image_path = os.path.join(doc_dir, 'figures', 'Full_slab_wood10.png')
        case 8:
        ...
        case 11:
            start_row = 9600
            image_path = os.path.join(doc_dir, 'figures', 'Full_slab_wood15.png')
        case 12:
            start_row = 9800
            image_path = os.path.join(doc_dir, 'figures', 'Full_slab_wood15.png')
        case 13:
        ...
        case 21:
            start_row = 9150
            image_path = os.path.join(doc_dir, 'figures', 'Trough_nowood.png')
        case 22:
            start_row = 9800
            image_path = os.path.join(doc_dir, 'figures', 'Trough_nowood.png')
        case 23:
        ...
        case 26:
            start_row = 9800
            image_path = os.path.join(doc_dir, 'figures', 'Trough_wood10.png')
        case 27:
            start_row = 9800
            image_path = os.path.join(doc_dir, 'figures', 'Trough_wood10.png')
        case 28:
        ...
        case 31:
            start_row = 8800
            image_path = os.path.join(doc_dir, 'figures', 'Trough_wood15.png')
        case 32:
            start_row = 8950
            image_path = os.path.join(doc_dir, 'figures', 'Trough_wood15.png')
        case 33:
        ...
        case 36:
            start_row = 9800
            image_path = os.path.join(doc_dir, 'figures', 'Trough_wood20.png')
        case 37:
            start_row = 9600
            image_path = os.path.join(doc_dir, 'figures', 'Trough_wood20.png')
        case 38:
    ...
    yticks = np.arange(int(min(min_ax)), int(max(max_ax)) + 1, step=10)
    plt.yticks(yticks)
    #plt.yticks(np.linspace(int(min_ax[0]), int(max_ax[0]), num=int((max_ax[0] - min_ax[0]) / 20) + 1))
    ...
    # plt.text(time_zm-0.25, min_ax[0]/7, '$\it{t_{F_{Z,max}}}$', fontsize=35, color='red')
    # plt.text(time_y+0.1, max_ax[0]/10, '$\it{t_{F_{Y,max}}}$', fontsize=35, color='green')
    # plt.text(time_z, max_ax[0]/10, '$\it{t_{F_{Z,min}}}$', fontsize=35, color='blue')
    ...
    arr_img = plt.imread(image_path)
    im = OffsetImage(arr_img)#, zoom=.45)
    ab = AnnotationBbox(im, (0.76, 0.919), xycoords='axes fraction', bboxprops=dict(facecolor='white', edgecolor='lightgrey', boxstyle='round,pad=0.5', alpha=0.8))
    ax.add_artist(ab)
    fig.tight_layout()
    plt.show
